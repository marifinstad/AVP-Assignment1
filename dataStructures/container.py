...
def NewShip(length, width, height):
    return [length, width, height, [], None]
...
def makeDeck(ship):
    d = [[[None]*getWidthShip(ship)]*getLengthShip(ship)]
...
def findContainer(ship, container):
    placement = []
    for i in range(getHeightShip(ship)):
        deck = ship[4][i]
        for j in range(getLengthShip(ship)):
...
def findPlace(ship, container):
    for i in range(getHeightShip(ship)):
        ...
        if getLengthContainer(container) == 20:
            ...
            for j in range(getLengthShip(ship)-2): ##-?
                row1 =  deck[j]
                row2 = deck [j + 1]
                for k in range(getWidthShip(ship)):
                    position1 = row1[k]
                    position2 = row2[k]
                    if (position1 == 0 and position2 == 0 and isContainerBelow(ship, [i, j, k]) and isContainerBelow(ship, [i, j+1, k])):
...
def isContainerBelow(ship, position):
    positionBelow = ship[4][position[0]-1][position[1]][position[2]]
    if positionBelow == None:
        return True
    elif positionBelow == 0:
        return False
    elif positionBelow != 0:
        return True

def addContainerOnShip(ship, container):
    position = findPlace(ship, container)
    if position == False:
        return False
    ship[3].append(container)
    for pos in position:
        ship[4][pos[0]][pos[1]][pos[2]] = container
    return True
